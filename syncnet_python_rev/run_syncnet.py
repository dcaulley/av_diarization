...
from SyncNetInstance_eg import *
...
s.loadParameters('syncnet_python/'+ opt.initial_model);
...
for ii, track in enumerate(tracks):
    offset, conf, dist, frame_conf = s.evaluate(opt,videofile=os.path.join(opt.crop_dir,opt.reference,'%05d.avi'%ii))
...
with open(os.path.join(opt.work_dir,opt.reference,'frame_confs.pckl'), 'wb') as fil:
