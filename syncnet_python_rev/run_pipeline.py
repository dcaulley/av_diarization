...
sys.path.append("/syncnet_python_rev")
...
parser.add_argument('--faces_file', type=str, default='', help='Directory to verified faces');
parser.add_argument('--scenes_file', type=str, default='', help='Directory to Scenes Detected');
parser.add_argument('--search_faces', type=int, default=1, help='0 if faces already found');
...
# ========== ========== ========== ==========
# # Creating the directories for pipeline
# ========== ========== ========== ==========
...
def crop_video(opt,track,cropfile):
  ...
  cap = cv2.VideoCapture(os.path.join(opt.avi_dir,'video.avi'))
  ...
  # ========== CROP AUDIO FILE ==========
  vid_in   = '"%s"'%os.path.join(opt.avi_dir,'video.avi')
...
def inference_video(opt):
  ...
  PATH_TO_CKPT = './syncnet_python_rev/protos/frozen_inference_graph_face.pb'

  # List of the strings that is used to add correct label for each box.
  PATH_TO_LABELS = './syncnet_python_rev/protos/face_label_map.pbtxt'
...
def scene_detect(opt):
  ...
  video_framerate, frames_read, scene_list = find_scenes(os.path.join(opt.avi_dir,'video.avi'))
  ...
  print('%s - scenes detected %d from %d frames'%(os.path.join(opt.avi_dir,'video.avi'),len(scene_list),frames_read))
...
savepath = os.path.join(opt.work_dir,'alltracks.pckl')
